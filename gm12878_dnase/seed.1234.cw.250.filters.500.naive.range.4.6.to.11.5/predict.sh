...
CUDA_VISIBLE_DEVICES=$gpu kerasAC_predict_tdb \
		    ...
		    --predictions_and_labels_hdf5 $outdir/predictions.gm12878.withdups.$seed\seed.$weight\counts.$fold \
		    --load_model_hdf5 $outdir/GM12878.profile.peaks.only.bpnet.withdups.$seed\seed.$weight\counts.$fold.hdf5 \
		    --tasks ENCSR000EMT \
		    --upsample_threads 1 \
