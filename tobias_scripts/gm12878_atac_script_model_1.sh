...
date=$(date +'%m.%d.%Y')
#setting=tobias_with_corrected_$data_type"_"$date
cur_file_name="gm12878_atac_script_model_1.sh"
#setting=tobias_with_corrected_ATAC_07.31.2021
setting=tobias_with_corrected_ATAC_08.02.2021/

### SIGNAL INPUT

corrected_bw=$PWD/tobias_scripts/$cell_line/$cell_line.atac.filt.merged_corrected.bw
...
data_dir=$PWD/tobias_scripts/$cell_line/tiledb_corrected
...
if [[ -d $data_dir ]] ; then
    echo "data director already exists"
else
    mkdir $data_dir
fi
...
if [[ -d $output_dir/model ]] ; then
    ...
    for fold in 0
    do
        ./tobias_scripts/main_scripts/model_1/train.sh $fold $gpu $model_name $seed $output_dir/model $params  $data_dir/tiledb/db $cell_line $PWD/tobias_scripts/main_scripts/model_1/profile_bpnet_dnase_with_bias.py
        ./tobias_scripts/main_scripts/model_1/predict.sh $fold $gpu $model_name $seed  $output_dir/model  $data_dir/tiledb/db $cell_line $chrom_sizes
        ./tobias_scripts/main_scripts/model_1/score.sh $output_dir/model $model_name $fold $cell_line $seed
    done
...
if [[ -d $output_dir/model/deepshap ]] ; then
    ...
    for fold in 0
    do
        ./main_scripts/tobias_scripts/interpret/interpret_weight.sh $output_dir/model/$model_name.$fold $bed_file xaa $data_dir/tiledb/db $chrom_sizes $output_dir/model/deepshap $cell_line $gpu $fold
        ./main_scripts/tobias_scripts/interpret/interpret_weight.sh $output_dir/model/$model_name.$fold $bed_file xab $data_dir/tiledb/db $chrom_sizes $output_dir/model/deepshap $cell_line $gpu $fold
        ./main_scripts/tobias_scripts/interpret/interpret_weight.sh $output_dir/model/$model_name.$fold $bed_file xac $data_dir/tiledb/db $chrom_sizes $output_dir/model/deepshap $cell_line $gpu $fold
    done
