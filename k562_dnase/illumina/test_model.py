...
def build1d_model_residual(input_width,input_dimension,number_of_convolutions,filters,filter_dim,dilation,activations,bn_true=True,max_flag=True):
    ...
    x = Conv1D(300,19,padding='same')(input1)
    x = BatchNormalization(axis=-1)(x)
    x = Activation('relu')(x)
    x = MaxPooling1D(pool_size=2)(x)

    x = Conv1D(200,11,padding='same')(x)
    x = BatchNormalization(axis=-1)(x)
    x = Activation('relu')(x)
    x = MaxPooling1D(pool_size=2)(x)
        
    conv=Conv1D(32,1, padding='same',activation='relu',name = 'upsampling')(x)
...
def getModelGivenModelOptionsAndWeightInits(args):
    ...
    input_dimension=4
    number_of_convolutions=1
    filters=[32,32,32] #[32,32,32,32,32,32,32,32,32,32,32,32,32,32,32,32]
    filter_dim=[11,21,41] #[11,11,11,11,11,11,11,11,21,21,21,21,41,41,41,41]
    dilation=[4,10,25] #[1,1,1,1,4,4,4,4,10,10,10,10,25,25,25,25]
    activations='relu'
    bn_true=True
