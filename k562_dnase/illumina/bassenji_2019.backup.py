...
def res_block(conv,num_filter,f_width,act,d_rate,i,bn_true=True):
    ...
    conv1 = Conv1D(768,1,padding="valid",name='conv_'+str(i)+'_b')(conv1)
    conv1 = BatchNormalization_mod(conv1,bn_true)
    conv1=Dropout(0.3)(conv1)
    print('done!')
    return keras.layers.Add()([conv1, crop])
...
def build1d_model_residual(input_width,input_dimension,number_of_convolutions,filters,filter_dim,dilation,activations,bn_true=True,max_flag=True):
    ...
    x = Conv1D(288,15,padding='same')(input1)
    x = BatchNormalization(axis=-1)(x)
    x = Activation('relu')(x)
    x = MaxPooling1D(pool_size=2)(x)
    
    x = Conv1D(339,5,padding='same')(input1)
    x = BatchNormalization(axis=-1)(x)
    x = Activation('relu')(x)
    x = MaxPooling1D(pool_size=2)(x)
    ...
    x = Conv1D(470,5,padding='same')(input1)
    x = BatchNormalization(axis=-1)(x)
    x = Activation('relu')(x)
    x = MaxPooling1D(pool_size=2)(x)

    x = Conv1D(554,5,padding='same')(input1)
    x = BatchNormalization(axis=-1)(x)
    x = Activation('relu')(x)
    x = MaxPooling1D(pool_size=2)(x)

    x = Conv1D(652,5,padding='same')(input1)
    x = BatchNormalization(axis=-1)(x)
    x = Activation('relu')(x)
    x = MaxPooling1D(pool_size=2)(x)
 
    x = Conv1D(768,5,padding='same')(input1)
    x = BatchNormalization(axis=-1)(x)
    x = Activation('relu')(x)
...
def getModelGivenModelOptionsAndWeightInits(args):
    ...
    dilation=[1,2,3,4,6,9,14,21,32,48,72]
    activations='relu'
    bn_true=True
