...
CUDA_VISIBLE_DEVICES=$gpu kerasAC_predict_tdb \
		    ...
		    --predictions_and_labels_hdf5 $outdir/predictions.k562.withdups.$seed\seed.$weight\counts.500filters.$fold \
		    --load_model_hdf5 $outdir/K562.profile.peaks.only.bpnet.withdups.$seed\seed.$weight\counts.500filters.$fold.hdf5 \
		    --tasks ENCSR000EOT \
		    --upsample_threads 1 \
