...
if [[ -d $output_dir/gm12878_bias_fit_on_signal_step2 ]] ; then
    ...
    bash $PWD/src/models/chrombpnet_scripts/get_loss_weights.sh $data_dir/tiledb/db "chr10" "overlap_peak" "count_bigwig_unstranded_5p" $cell_line $flank_size $output_dir/gm12878_bias_fit_on_signal_step2/counts_loss_weight.txt
...
if [[ -d $output_dir/with_gm12878_bias_final_model ]] ; then
    ...
    bash $PWD/src/models/chrombpnet_scripts/get_loss_weights.sh $data_dir/tiledb/db "chr10" "overlap_peak" "count_bigwig_unstranded_5p" $cell_line $flank_size $output_dir/with_gm12878_bias_final_model/counts_loss_weight.txt
    counts_loss_weight_step3=`cat $output_dir/with_gm12878_bias_final_model/counts_loss_weight.txt`
...
#CUDA_VISIBLE_DEVICES=$gpu python  $PWD/src/evaluation/marginal_footrprints/main_footprints_check.py --ref_fasta $ref_fasta --gc_neg $neg_bed_test --motifs tn5_c --model_dir $output_dir/gm12878_bias_fit_on_signal_step2/
