...
if [[ -d $output_dir/invivo_bias_model_step1 ]] ; then
    ...
    then
        ...
        for fold in 0
        do
            bash $PWD/src/models/chrombpnet_scripts/invivo_bias_model_step1/train.sh $fold $gpu $model_name $seed $output_dir/invivo_bias_model_step1 $params  $data_dir/tiledb/db $cell_line bpnet_model.py $neg_bed_train
...
if [[ -d $output_dir/final_model_step3 ]] ; then
    ...
    for fold in 0
    do
        $PWD/src/models/chrombpnet_scripts/final_model_step3/train.sh $fold $gpu $model_name $seed $output_dir/final_model_step3 $params  $data_dir/tiledb/db $cell_line $PWD/main_scripts/final_model_step3/bpnet_with_bias.py
        $PWD/src/models/chrombpnet_scripts/final_model_step3/predict.sh $fold $gpu $model_name $seed  $output_dir/final_model_step3  $data_dir/tiledb/db $cell_line $chrom_sizes
        $PWD/src/models/chrombpnet_scripts/final_model_step3/score.sh $output_dir/final_model_step3 $model_name $fold $cell_line $seed
    done
...
if [[ -d $output_dir/final_model_step3_new/unplug/deepshap ]] ; then
    ...
    for fold in 0
    do
        bash $PWD/src/evaluation/interpret/interpret.sh $output_dir/final_model_step3_new/unplug/$model_name.$fold.hdf5 $bed_file xaa $data_dir/tiledb/db $chrom_sizes $output_dir/final_model_step3_new/unplug/deepshap $cell_line $gpu $fold
        bash $PWD/src/evaluation/interpret.sh $output_dir/final_model_step3_new/unplug/$model_name.$fold.hdf5 $bed_file xab $data_dir/tiledb/db $chrom_sizes $output_dir/final_model_step3_new/unplug/deepshap $cell_line $gpu $fold
        bash $PWD/src/evaluation/interpret.sh $output_dir/final_model_step3_new/unplug/$model_name.$fold.hdf5 $bed_file xac $data_dir/tiledb/db $chrom_sizes $output_dir/final_model_step3_new/unplug/deepshap $cell_line $gpu $fold
    done

    python $PWD/src/evaluation/interpret/combine_shap_pickle.py --source $output_dir/final_model_step3_new/unplug/deepshap --target $output_dir/final_model_step3_new/unplug/deepshap --type 20k
...
if [[ -d $modisco_sig_dir/$cell_line/$setting"_new"/ ]] ; then
    ...
    cp  $output_dir/final_model_step3_new/unplug/deepshap/20K.fold0.deepSHAP $modisco_dir_final
fi
