...
def main():
    ...
    regions=pd.read_csv(args.bed_file_to_score,header=None,sep='\t')
    ...
    for index,row in regions.iterrows(): 
        ...
        start_pos_input_bigwig=start_pos_input
        start_pos_output_bigwig=start_pos_output
        delta_input=0
        delta_output=0 
        if last_chrom is not None:
            if chrom==last_chrom:
                #check for overlap of bigwig coordinates
                if start_pos_input < last_end_input:
                    start_pos_input_bigwig=last_end_input
                    delta_input=last_end_input-start_pos_input
                if start_pos_output < last_end_output:
                    start_pos_output_bigwig=last_end_output
                    delta_output=last_end_output-start_pos_output
        last_chrom=chrom
        last_end_input=end_pos_input
        last_end_output=end_pos_output
        #print(str(chrom)+":"+str(start_pos_output))
        #get the reference and alternate one-hot-encoded sequences 
        seq=ref.fetch(chrom,summit-args.flank_input,summit+args.flank_input)
        onehot=one_hot_encode([seq])

        #get the bigwig labels 
        labels=np.nan_to_num(pbw.values(chrom,summit-args.flank_output,summit+args.flank_output))
        #print(labels.shape)
        outputs['observed_count_bigwig'].addEntries(chrom,start_pos_output_bigwig,values=labels[delta_output::], span=1,step=1)
        ...
        outputs['predicted_count_bigwig'].addEntries(chrom,start_pos_output_bigwig,values=predicted_count_track[delta_output::],span=1,step=1) 
        outputs['predicted_probability_bigwig'].addEntries(chrom,start_pos_output_bigwig,values=predicted_prob_track[delta_output::],span=1,step=1)
        ...
        outputs['deepshap_profile'].addEntries(chrom,start_pos_input_bigwig,values=profile_explanations_shap[delta_input::],span=1,step=1)
        outputs['deepshap_counts'].addEntries(chrom,start_pos_input_bigwig,values=count_explanations_shap[delta_input::],span=1,step=1)
        ...
        if args.run_ism ==True:
            ...
            outputs['ism_profile'].addEntries(chrom,start_pos_input_bigwig,values=single_bp_ism_profile_track_adjusted[delta_input::],span=1,step=1)
            outputs['ism_count'].addEntries(chrom,start_pos_input_bigwig,values=single_bp_ism_count_track_adjusted[delta_input::],span=1,step=1) 
    #close the files
    for key in outputs:
        outputs[key].close() 
