...
def resolve_hits(filtered_hits_path, collapsed_hits_path):
    """
    Collapses hits by merging instances of the same motif that overlap.
    """
    # For each PFM key, merge all its hits, collapsing strand, score, and peak
    ...
    f.close()
    with open(temp_file, "a") as f:
        comm = ["bedtools", "cluster", "-i", filtered_hits_path]
        proc = subprocess.Popen(" ".join(comm), shell=True, stdout=f)
        proc.wait()
    ...
    new_data = []
    print(data.head())
    clusters = list(set(data[7].values))
    print(data[7].values)
    print(len(clusters))
    print(data.shape)
    ...
    for idx in tqdm(clusters):
        obc = data[data[7]==idx].reset_index(drop=True)
        if len(obc) == 1:
            new_data.append(obc.loc[0,:].values[0:-1])
        elif len(obc) == 0:
            continue
        else:
            while len(obc) > 0:
                idx = obc[5].idxmax()
                new_data.append(obc.loc[idx,:].values[0:-1])
    ...
    new_data.to_csv(collapsed_hits_path ,sep="\t", header=False, index=False)
...
def get_moods_hits(
    ...
    print("Collapse overlapping hits...")
    ...
    #print("Resolve overlapping hits...")
    # Resolve overlapping hits of the same motif
    #resolve_hits(
    #    os.path.join(temp_dir, "moods_filtered_collapsed.bed"),
    #    os.path.join(temp_dir, "moods_filtered_collapsed_overlap_resolve.bed")
    #)
    ...
    compute_hits_importance_scores(
        ...
        os.path.join(temp_dir, "moods_filtered_collapsed_scored.bed"), method
    )
...
def get_moods_hits_from_collapsed(
    pfm_dict, reference_fasta, peak_bed_path, shap_scores_hdf5_path, peak_table_in,
