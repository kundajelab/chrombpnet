...
def get_deepshap(prof_explainer,count_explainer,seq_onehot, control_counts):
    #dim will be (1,2114,4)
    #hyp_profile_explanations=prof_explainer.shap_values([seq_onehot, np.zeros((len(control_counts),1000)) , np.array(control_counts)])
    #hyp_count_explanations=count_explainer.shap_values([seq_onehot, np.zeros((len(control_counts),1000)) , np.array(control_counts)])[0]
...
def get_preds(model,seq_onehot, control_counts):
    print(np.array(seq_onehot).shape)
    preds=model.predict([seq_onehot, np.zeros((len(control_counts),1000,1)) , np.array(control_counts)])
    prof=preds[0] #logits
    ...
    #print(np.exp(count)+(control_counts)*1.0)
    #print(np.exp(count+np.log(np.array(control_counts)+1)))
    #print(np.exp(count))
...
def main():
    ...
    print("loaded model") 
    pbw1=pyBigWig.open("/srv/scratch/anusri/chrombpnet_paper/tobias_scripts/GM12878/GM12878.atac.filt.merged_expected.bw", 'r')
    ...
    for chrom in chroms:
        ...
        for batch in np.array_split(regions_chrom,num_batches):
            ...
            control_counts=[np.log(np.sum(np.nan_to_num(pbw1.values(chrom,start_pos_output[i],end_pos_output[i])))+0.001) for i in range(cur_batch_size)]
            onehot=one_hot_encode(seq)

            #profile head out, count head out, probability predicted track, count predicted track 
            predicted_profile_logit_head,predicted_count_head,predicted_prob_track,predicted_count_track=get_preds(model,onehot, control_counts)

            #get deepSHAP scores  
            #hyp_profile_explanations_shap, hyp_count_explanations_shap,profile_explanations_shap, count_explanations_shap=get_deepshap(prof_explainer, count_explainer, onehot, control_counts)
