...
for chrm in chrms:
    ...
    if os.path.isfile(args.out_prefix + "_" + chrm +"_bias.stats.txt"):
        print(args.out_prefix + "_" + chrm +"_bias.stats.txt")
        continue
    ...
    bigwig_helper.write_bigwig(softmax(pred_logits) * (np.expand_dims(np.exp(pred_logcts)[:,0],axis=1)),
                           regt,
                           gs,
                           args.out_prefix + "_" + chrm + "_bias.bw",
                           args.out_prefix + "_" + chrm +"_bias.stats.txt",
