...
def parse_args():
    ...
    parser.add_argument("-p","--path_to_pickle")
...
def main():
    ...
    #print(keys)
    #myList = [(int(key.split("_")[0].replace("chr","").replace("X","23").replace("Y", "24")), int(key.split("_")[1])) for key in keys]
    myList = [(key.split("_")[0], int(key.split("_")[1])) for key in keys]
    #print(myList)
    ...
    for key_idx in sorted_idx:
        ...
        print(profile_preds[key].keys())
        ref_profile_prof = np.array(softmax(profile_preds[key]['ref'],axis=0))
        ref_counts = count_preds.loc[key, 'ref']
        ref_profile = ref_profile_prof*np.expand_dims(np.exp([ref_counts]), axis=1)[0]

        alt_profile_prof = np.array(softmax(profile_preds[key]['alt'],axis=0))
        ...
        if Pos0-int(prof_flank) >= cur_pos:
            print(chrom, Pos0-int(prof_flank), len(ref_profile))
            #print(ref_profile)
