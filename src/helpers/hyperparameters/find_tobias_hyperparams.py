...
def get_seqs_cts(genome, bw, peaks_df, input_width=2114, output_width=1000):
    ...
    return (np.sum(np.array(vals),axis=1), one_hot.dna_to_one_hot(seqs), np.array(vals))
...
def parse_data_args():
    ...
    parser.add_argument("-bb", "--bias_bigwig", type=str, required=True, help="Bigwig of tn5 insertions. Ensure it is +4/-4 shifted")
...
def adjust_bias_model_logcounts(bias_model, seqs, cts, bias_cts, bias_logcts):
    """
    ...
    _, pred_logcts = bias_model.predict([np.expand_dims(bias_cts,axis=-1),bias_logcts], verbose=True)
...
if __name__=="__main__":
    ...
    bias_bw = pyBigWig.open(args.bias_bigwig) 
    ...
    peak_cnts, _, _ = get_seqs_cts(genome, bw, peaks, args.inputlen, args.outputlen)
    nonpeak_cnts, nonpeak_seqs, _ = get_seqs_cts(genome, bw, nonpeaks, args.inputlen, args.outputlen)    
    bias_nonpeak_sum_cnts, _, bias_nonpeak_cnts = get_seqs_cts(genome, bias_bw, nonpeaks, args.inputlen, args.outputlen+100)    

    assert(bias_nonpeak_sum_cnts.shape[0]==nonpeak_cnts.shape[0])
    print(bias_nonpeak_cnts.shape)
    print(bias_nonpeak_sum_cnts.shape)
    ...
    bias_model_scaled = adjust_bias_model_logcounts(bias_model, nonpeak_seqs[(nonpeak_cnts< upper_thresh) & (nonpeak_cnts>lower_thresh)], nonpeak_cnts[(nonpeak_cnts< upper_thresh) & (nonpeak_cnts>lower_thresh)], bias_nonpeak_cnts[(nonpeak_cnts< upper_thresh) & (nonpeak_cnts>lower_thresh)], np.log(bias_nonpeak_sum_cnts+1)[(nonpeak_cnts< upper_thresh) & (nonpeak_cnts>lower_thresh)])
