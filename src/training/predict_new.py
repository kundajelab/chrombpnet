...
def main():
    ...
    mnll_pw, mnll_norm, jsd_pw, jsd_norm, jsd_rnd, jsd_rnd_norm, mnll_rnd, mnll_rnd_norm =  metrics.profile_metrics_new(true_counts,profile_probs_predictions)
    ...
    if args.peaks != "None" and args.nonpeaks != "None":
        ...
        gt_np = true_counts_sum[non_peaks_idx]
        pd_np = counts_sum_predictions[non_peaks_idx]

        gt_p = true_counts_sum[peaks_idx]
        pd_p = counts_sum_predictions[peaks_idx]

        gt_scores = [1]*len(gt_p) + [0]*len(gt_np)
        print(pd_p.shape)
        print(pd_np.shape)
        print(len(gt_p))
        pred_scores = np.hstack([pd_p, pd_np])
        print(pred_scores.shape)
        print(len(gt_scores))

        auprc = sklearn.metrics.average_precision_score(gt_scores, pred_scores)
        auroc = sklearn.metrics.roc_auc_score(gt_scores, pred_scores)

        metrics_dictionary["counts_metrics"]["peaks_and_nonpeaks"]["auprc"] = auprc
        metrics_dictionary["counts_metrics"]["peaks_and_nonpeaks"]["auroc"] = auroc
