...
class ChromBPNetBatchGenerator(keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        if self.return_coords:
            return ([np.expand_dims(batch_bias_cts,axis=-1), np.log(1+batch_bias_cts.sum(-1, keepdims=True))], [batch_cts, np.log(1+batch_cts.sum(-1, keepdims=True))], batch_coords)
        else:
            return ([np.expand_dims(batch_bias_cts,axis=-1), np.log(1+batch_bias_cts.sum(-1, keepdims=True))], [batch_cts, np.log(1+batch_cts.sum(-1, keepdims=True))])
