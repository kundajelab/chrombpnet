...
from utils import bias_augment as augment
...
def subsample_nonpeak_data(nonpeak_seqs, nonpeak_cts, bias_nonpeak_cts, nonpeak_coords, peak_data_size, negative_sampling_ratio):
    ...
    bias_nonpeak_cts = bias_nonpeak_cts[nonpeak_indices_to_keep]
    nonpeak_coords = nonpeak_coords[nonpeak_indices_to_keep]
    return nonpeak_seqs, nonpeak_cts, bias_nonpeak_cts, nonpeak_coords
...
class ChromBPNetBatchGenerator(keras.utils.Sequence):
    ...
    """
    def __init__(self, peak_regions, nonpeak_regions, genome_fasta, batch_size, inputlen, outputlen, max_jitter, negative_sampling_ratio, cts_bw_file, cts_bias_bw_file, add_revcomp, return_coords, shuffle_at_epoch_start):
        """
        seqs: B x L' x 4
        cts: B x M'
        ...
        self.bias_peak_cts, self.bias_nonpeak_cts = bias_peak_cts, bias_nonpeak_cts
    ...
    def crop_revcomp_data(self):
        ...
        if (self.peak_seqs is not None) and (self.nonpeak_seqs is not None):
            ...
            cropped_peaks, cropped_cnts, cropped_bias_cnts, cropped_coords = augment.random_crop(self.peak_seqs, self.peak_cts, self.bias_peak_cnts, self.inputlen, self.outputlen,self.outputlen, self.peak_coords)
            #print(cropped_peaks.shape)
            #print(self.nonpeak_seqs.shape)
            if self.negative_sampling_ratio < 1.0:
                self.sampled_nonpeak_seqs, self.sampled_nonpeak_cts, self.bias_sampled_nonpeak_cts, self.sampled_nonpeak_coords = subsample_nonpeak_data(self.nonpeak_seqs, self.nonpeak_cts, self.bias_nonpeak_cts, self.nonpeak_coords, len(self.peak_seqs), self.negative_sampling_ratio)
                self.seqs = np.vstack([cropped_peaks, self.sampled_nonpeak_seqs])
                self.cts = np.vstack([cropped_cnts, self.sampled_nonpeak_cts])
                self.bias_cts = np.vstack([cropped_bias_cnts, self.bias_sampled_nonpeak_cts])
                ...
                self.bias_cts = np.vstack([cropped_bias_cnts, self.bias_nonpeak_cts])
            ...
            self.bias_cts = cropped_bias_cnts
            self.coords = cropped_coords

            #assert(np.all(self.bias_cts>=0))
        elif self.nonpeak_seqs is not None:
            #print(self.nonpeak_seqs.shape)

            self.seqs = self.nonpeak_seqs
            self.cts = self.nonpeak_cts
            self.bias_cts = self.bias_nonpeak_cts
            self.coords = self.nonpeak_coords
        else :
            print("Both peak and non-peak arrays are empty")

        self.cur_seqs, self.cur_cts, self.bias_cur_cts, self.cur_coords = augment.crop_revcomp_augment(
                                            self.seqs, self.cts, self.bias_cts, self.coords, self.inputlen, self.outputlen, 
    ...
    def __getitem__(self, idx):
        ...
        if self.return_coords:
            return ([batch_seq], [((batch_bias_cts)/(batch_bias_cts).sum(-1, keepdims=True))*batch_cts.sum(-1, keepdims=True), np.log(1+batch_cts.sum(-1, keepdims=True))], batch_coords)
        else:
            return ([batch_seq], [((batch_bias_cts)/(batch_bias_cts).sum(-1, keepdims=True))*batch_cts.sum(-1, keepdims=True), np.log(1+batch_cts.sum(-1, keepdims=True))])
