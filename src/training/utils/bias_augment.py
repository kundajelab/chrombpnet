...
def random_crop(seqs, labels, bias_labels, seq_crop_width, label_crop_width, bias_crop_width, coords):
    """
    ...
    assert(bias_labels.shape[1]>=bias_crop_width)
    ...
    return take_per_row(seqs, starts, seq_crop_width), take_per_row(labels, starts, label_crop_width), take_per_row(bias_labels, starts, bias_crop_width), new_coords

def random_rev_comp(seqs, labels, bias_labels, coords, frac=0.5):
    """
    ...
    bias_labels[pos_to_rc] = bias_labels[pos_to_rc, ::-1]
    coords[pos_to_rc,2] =  "r"
	
    return seqs, labels, bias_labels, coords

def crop_revcomp_augment(seqs, labels, bias_labels, coords, seq_crop_width, label_crop_width, add_revcomp, rc_frac=0.5, shuffle=False):
    """
    seqs: B x IL x 4
    labels: B x OL
    ...
    mod_seqs, mod_labels, mod_bias_labels, mod_coords = seqs, labels, bias_labels, coords

    # this modifies mod_seqs, mod_labels in-place
    if add_revcomp:
        mod_seqs, mod_labels, mod_bias_labels, mod_coords = random_rev_comp(mod_seqs, mod_labels, mod_bias_labels, mod_coords, frac=rc_frac)
    ...
    if shuffle:
        ...
        mod_bias_labels = mod_bias_labels[perm]
        mod_coords = mod_coords[perm]


    return mod_seqs, mod_labels, mod_bias_labels, mod_coords
