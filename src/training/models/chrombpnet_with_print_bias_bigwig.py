...
def getModelGivenModelOptionsAndWeightInits(args, model_params):   
    ...
    inp = Input(shape=(sequence_len, 4),name='sequence')    
    inp_bias_logits = Input(shape=(out_pred_len,1), name="bias_logits")
    ...
    #bias_output=
    #cropsize = int((out_pred_len+100)/2)-int(out_pred_len/2)
    #assert cropsize>=0
    #assert (cropsize % 2 == 0) # Necessary for symmetric cropping

    #bias_output = Cropping1D(cropsize,
    #            name='wo_bias_bpnet_logitt_before_flatten')(inp_bias_logits)

    bias_output = inp_bias_logits
    ...
    assert(bias_output.shape[1]==out_pred_len) # bias model profile head is of incorrect shape (None,out_pred_len) $
    #print("here")
    #print(output_wo_bias[0].shape)
    #print(bias_output.shape)
    #print(bias_output[:,:])
    bias_output_squeezed = Lambda(lambda x: squeeze(x, axis=-1))(bias_output)
    #print(bias_output_squeezed.shape)

    profile_out = Add(name="logits_profile_predictions")([output_wo_bias[0], bias_output_squeezed])
    ...
    #print(count_out.shape)
    #print(profile_out.shape)
    # instantiate keras Model with inputs and outputs
    model=Model(inputs=[inp, inp_bias_logits],outputs=[profile_out, count_out])
