...
def parse_args():
    ...
    parser.add_argument("-c","--chr",type=str, required=True, help="chromosome to build pwm")
    parser.add_argument("-cz","--chr_size",type=int, required=True, help="chromosome size to consider")
...
if __name__=="__main__":
    ...
    seq = str(hg38[args.chr][0:args.chr_size])
    one_hot_seq = one_hot.dna_to_one_hot(seq).squeeze()
    bigwig_vals = np.nan_to_num(bw.values(args.chr,0,args.chr_size ))
    print("non zero entries in the given chromosome: ", np.sum(bigwig_vals>0))
