...
negatives_dir=results/chrombpnet/$data_type/$cell_line/negatives_data/negatives_with_summit.bed
...
if [[ -d $output_dir/uncorrected_model/interpret ]] ; then
    ...
    echo $( timestamp ):"python $PWD/src/evaluation/interpret/interpret.py \
        --genome=$ref_fasta \
        --regions=results/chrombpnet/$data_type/$cell_line/data/30K.subsample.overlap.bed \
