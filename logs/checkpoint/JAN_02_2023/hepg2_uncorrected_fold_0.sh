...
date=$(date +'%m.%d.%Y')
cur_file_name="hepg2_uncorrected_atac_fold_0.sh"
### SIGNAL INPUT
...
overlap_peak=results/chrombpnet/ATAC_PE/HEPG2/data/peaks_no_blacklist.bed
negatives_dir=results/chrombpnet/ATAC_PE/HEPG2/negatives_data/negatives_with_summit.bed
...
if [[ -d $output_dir/uncorrected_model/interpret ]] ; then
    ...
    echo $( timestamp ):"python $PWD/src/evaluation/interpret/interpret.py \
        --genome=$ref_fasta \
        --regions=results/chrombpnet/ATAC_PE/HEPG2/data/30K.subsample.overlap.bed \
