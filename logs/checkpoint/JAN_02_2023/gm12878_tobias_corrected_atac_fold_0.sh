...
date=$(date +'%m.%d.%Y')
#setting=$data_type"_"$date"_tobias_corrected_softmax"
setting=$data_type"_"$date"_tobias_corrected_custom_shift"
cur_file_name="gm12878_tobias_corrected_atac_fold_0.sh"
### SIGNAL INPUT

in_bigwig=results/chrombpnet/ATAC_PE/GM12878/data/GM12878_unstranded.bw
bias_bigwig=results/tobias/ATAC_PE/GM12878/data_new/GM12878_orig/sorted_merged_corrected.bw
overlap_peak=$PWD/results/chrombpnet/ATAC_PE/GM12878/data/peaks_no_blacklist.bed
#overlap_peak=test.bed
...
if [[ -d $output_dir/tobias_model/interpret ]] ; then
    ...
    echo $( timestamp ):"python $PWD/src/evaluation/interpret/interpret.py \
        ...
        --model_h5=$output_dir/tobias_model/tobias.h5" |  tee -a $logfile
