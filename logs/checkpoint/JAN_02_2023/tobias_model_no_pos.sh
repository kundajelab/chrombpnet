...
if [ -z "$logfile" ]
  then
    ...
    logfile=$output_dir"/train_bias_model_preds.log"
    touch $logfile
fi

echo $( timestamp ): "python $PWD/src/helpers/hyperparameters/find_tobias_hyperparams.py \\
...
python $PWD/src/helpers/hyperparameters/find_tobias_hyperparams.py \
       --genome=$reference_fasta \
       --bigwig=$bigwig_path \
       --bias_bigwig=$bias_bigwig \
       --peaks=$overlap_peak \
...
echo $( timestamp ): "python $PWD/src/training/with_bias_train.py \\
       --genome=$reference_fasta \\
       --bigwig=$bigwig_path \\
       --bias_bigwig=$bias_bigwig \\
       --peaks=$output_dir/filtered.peaks.bed  \\
       --params=$output_dir/chrombpnet_model_params.tsv \\
       --output_prefix=$output_dir/tobias \\
       --chr_fold_path=$fold \\
       --batch_size=64 \\
       --architecture_from_file=$PWD/src/training/models/chrombpnet_with_bias_bigwig.py \\
...
python $PWD/src/training/with_bias_train.py \
       --genome=$reference_fasta \
       --bigwig=$bigwig_path \
       --bias_bigwig=$bias_bigwig \
       --peaks=$output_dir/filtered.peaks.bed  \
...
echo $( timestamp ): "python $PWD/src/training/predict.py \\
        ...
        --output_prefix=$output_dir/tobias \\
        --batch_size=256 \\
        --model_h5=$output_dir/tobias_wo_bias.h5" | tee -a $logfile
...
if [[ "$data_type" = "DNASE_SE" || "$data_type" = "DNASE_PE" ]] ; then
        ...
        --chr_fold_path=$fold \\
        -m $output_dir/tobias_wo_bias.h5 \\
        -bs 512 \\
