...
date=$(date +'%m.%d.%Y')
cur_file_name="k562_negatives_create_nps_1.sh"
### SIGNAL INPUT
...
neg_dir=$main_dir/testing_1to1_negatives_data
inputlen=2114
gpu=1
...
if [[ -d $neg_dir ]] ; then
    ...
    mkdir $neg_dir
    bash testing_step3_get_background_regions.sh $ref_fasta $chrom_sizes $blacklist_region $overlap_peak $inputlen $genomewide_gc $neg_dir $fold 
fi
