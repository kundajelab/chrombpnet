...
output_dir=results/chrombpnet/ATAC_PE/GM12878/data/
##name sort to achieve random genomic pos#
#samtools sort -n --threads 50 -O bam -o $input_bam.namesorted.bam $input_bam
##echo "done sorting" 
##samtools view -H $input_bam > $output_dir/pr1.sam
#cp $output_dir/pr1.sam $output_dir/pr2.sam
##echo "made headers"
#sam1=$output_dir/pr1.sam
#sam2=$output_dir/pr2.sam 
#samtools view --threads 50  $input_bam | awk -v sam1="$sam1" -v sam2="$sam2" '{if(NR%2){print >> sam1} else {print >> sam2}}'

#samtools view -bS $sam1 > $output_dir/pr1.bam
#samtools view -bS $sam2 > $output_dir/pr2.bam
