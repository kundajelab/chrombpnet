...
setting=$data_type"_"$date"_naked_dna_simplebias_45_shift_"$bias_threshold_factor
cur_file_name="gm12878_simple_bias_45_shift.sh"
#setting=ATAC_PE_03.12.2022_naked_dna_simplebias_45_shift_1.0
### SIGNAL INPUT
...
setting1=ATAC_PE_03.12.2022_simplebias_45_shift_1.0

if [[ -d $oak_dir/$setting1/ ]]; then
    echo "dir exists"
else
    mkdir $oak_dir/$setting1/
    mkdir $oak_dir/$setting1/SIGNAL
    mkdir $oak_dir/$setting1/BIAS
fi

if [[ -f $oak_dir/$setting1/BIAS/$cell_line.counts_scores.h5  && -f $oak_dir/$setting1/BIAS/$cell_line.profile_scores.h5 ]] ; then
    echo "bias model files already present on oak"
else
    echo "copying bias model counts and profile scores to oak"
    cp $output_dir/bias_model/interpret/$cell_line.counts_scores.h5 $oak_dir/$setting1/BIAS/
    cp $output_dir/bias_model/interpret/$cell_line.profile_scores.h5 $oak_dir/$setting1/BIAS/
fi


if [[ -f $oak_dir/$setting1/SIGNAL/$cell_line.counts_scores.h5  && -f $oak_dir/$setting1/SIGNAL/$cell_line.profile_scores.h5 ]] ; then
    echo "chrombpnet model files already present on oak"
else
    echo "copying counts and profile scores to oak"
    cp $output_dir/chrombpnet_model/interpret/$cell_line.counts_scores.h5 $oak_dir/$setting1/SIGNAL/
    cp $output_dir/chrombpnet_model/interpret/$cell_line.profile_scores.h5 $oak_dir/$setting1/SIGNAL/
fi
