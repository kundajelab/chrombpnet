...
in_bigwig=results/hint_atac/DNASE_SE/GM12878/data/GM12878_unstranded.bw
overlap_peak=$PWD/results/chrombpnet/DNASE_SE/GM12878/data/peaks_no_blacklist.bed
nonpeaks=$PWD/results/chrombpnet/DNASE_SE/GM12878/negatives_data/negatives_with_summit.bed 
...
if [[ -d $output_dir/hint_atac_model ]] ; then
    ...
    CUDA_VISIBLE_DEVICES=$gpu bash hint_atac_model_no_pos.sh $ref_fasta $in_bigwig $overlap_peak $nonpeaks $fold $output_dir/hint_atac_model $data_type
fi
