...
setting=$data_type"_"$date"_lite_bias"
...
bias_model="/mnt/lab_data3/anusri/chrombpnet-lite/pretrained/atac.bias.2114.1000.h5"
...
if [[ -d $output_dir/chrombpnet_model ]] ; then
    ...
    CUDA_VISIBLE_DEVICES=$gpu bash step6_train_chrombpnet_model.sh $ref_fasta $data_dir"/"$cell_line"_unstranded.bw" $overlap_peak $neg_dir/negatives_with_summit.bed $fold $bias_model $output_dir/chrombpnet_model $data_type
fi
