...
date=$(date +'%m.%d.%Y')
setting=4_$neg_shift"_shifted_"$data_type"_"$date"_withgm12878bias_newdata"
cur_file_name="withgm12878bias_k562_atac.sh"
setting=4_4_shifted_ATAC_10.26.2021_withgm12878bias_newdata
### SIGNAL INPUT
...
bias_weights="/srv/scratch/anusri/chrombpnet_paper/results/chrombpnet/ATAC/GM12878/ATAC_10.25.2021_withinvivobias/invivo_bias_model_step1/model.0.weights"
bias_json="/srv/scratch/anusri/chrombpnet_paper/results/chrombpnet/ATAC/GM12878/ATAC_10.25.2021_withinvivobias/invivo_bias_model_step1/model.0.arch"

if [[ -d $output_dir/gm12878_bias_fit_on_signal_step2 ]] ; then
    echo "skipping step 2"
else
    mkdir $output_dir/gm12878_bias_fit_on_signal_step2

    bash $PWD/src/models/chrombpnet_scripts/get_loss_weights.sh $data_dir/tiledb/db "chr10" "overlap_peak" "count_bigwig_unstranded_5p" $cell_line $flank_size $output_dir/gm12878_bias_fit_on_signal_step2/inpeaks_counts_loss_weight.txt
    bash $PWD/src/models/chrombpnet_scripts/get_loss_weights.sh $data_dir/tiledb/db "chr10" "negatives_peak" "count_bigwig_unstranded_5p" $cell_line $flank_size $output_dir/gm12878_bias_fit_on_signal_step2/innegs_counts_loss_weight.txt
    counts_loss_weight_step2=`head -n 1 $output_dir/gm12878_bias_fit_on_signal_step2/inpeaks_counts_loss_weight.txt`
    counts_threshold=`tail -n 1 $output_dir/gm12878_bias_fit_on_signal_step2/inpeaks_counts_loss_weight.txt`
    counts_loss_weight_step3=`head -n 1 $output_dir/gm12878_bias_fit_on_signal_step2/inpeaks_counts_loss_weight.txt`
    echo $counts_threshold
    echo -e "json_string\t"$bias_json"\nweights\t"$bias_weights"\ncounts_loss_weight\t"$counts_loss_weight_step2"\nprofile_loss_weight\t1\nfilters\t"$filters"\nn_dil_layers\t"$n_dil_layers > $output_dir/gm12878_bias_fit_on_signal_step2/params.txt
...
step2_bias_json=$output_dir/gm12878_bias_fit_on_signal_step2/model.0.arch
step2_bias_weights=$output_dir/gm12878_bias_fit_on_signal_step2/model.0.weights

if [[ -d $output_dir/with_gm12878_bias_final_model ]] ; then
    echo "skipping step 3"
else
    mkdir $output_dir/with_gm12878_bias_final_model
    echo -e "json_string\t"$step2_bias_json"\nweights\t"$step2_bias_weights"\ncounts_loss_weight\t"$counts_loss_weight_step3"\nprofile_loss_weight\t1\nfilters\t"$filters"\nn_dil_layers\t"$n_dil_layers > $output_dir/with_gm12878_bias_final_model/params.txt
...
if [[ -d $output_dir/with_gm12878_bias_final_model/unplug ]] ; then
    echo "skipping unplugging"
else
    mkdir $output_dir/with_gm12878_bias_final_model/unplug
    unplug_bias_json=$output_dir/with_gm12878_bias_final_model/model.0.arch
    unplug_bias_weights=$output_dir/with_gm12878_bias_final_model/model.0.weights
    echo -e "json_string\t"$unplug_bias_json"\nweights\t"$unplug_bias_weights"\ncounts_loss_weight\t"$counts_loss_weight_step3"\nprofile_loss_weight\t1\nfilters\t"$filters"\nn_dil_layers\t"$n_dil_layers > $output_dir/with_gm12878_bias_final_model/unplug/params.txt

    params=$output_dir/with_gm12878_bias_final_model/unplug/params.txt
...
if [[ -d $output_dir/with_gm12878_bias_final_model/unplug/deepshap ]] ; then
    ...
    for fold in 0
    do
        bash $PWD/src/evaluation/interpret/interpret.sh $output_dir/with_gm12878_bias_final_model/unplug/$model_name.$fold.hdf5 $bed_file xaa $data_dir/tiledb/db $chrom_sizes $output_dir/with_gm12878_bias_final_model/unplug/deepshap $cell_line $gpu $fold
        bash $PWD/src/evaluation/interpret/interpret.sh $output_dir/with_gm12878_bias_final_model/unplug/$model_name.$fold.hdf5 $bed_file xab $data_dir/tiledb/db $chrom_sizes $output_dir/with_gm12878_bias_final_model/unplug/deepshap $cell_line $gpu $fold
        bash $PWD/src/evaluation/interpret/interpret.sh $output_dir/with_gm12878_bias_final_model/unplug/$model_name.$fold.hdf5 $bed_file xac $data_dir/tiledb/db $chrom_sizes $output_dir/with_gm12878_bias_final_model/unplug/deepshap $cell_line $gpu $fold
    done

    python $PWD/src/evaluation/interpret/combine_shap_pickle.py --source $output_dir/with_gm12878_bias_final_model/unplug/deepshap --target $output_dir/with_gm12878_bias_final_model/unplug/deepshap --type 20k
    cp $PWD/$cur_file_name $output_dir/with_gm12878_bias_final_model/unplug/deepshap
...
if [[ -d $output_dir/with_gm12878_bias_final_model/deepshap ]] ; then
    echo "skipping interpretations"
else
    mkdir $output_dir/with_gm12878_bias_final_model/deepshap
    bed_file=$PWD/results/chrombpnet/$data_type/$cell_line/data/$cell_line"_idr_split"

    for fold in 0
    do
        bash $PWD/src/evaluation/interpret/interpret_weight.sh $output_dir/with_gm12878_bias_final_model/$model_name.$fold $bed_file xaa $data_dir/tiledb/db $chrom_sizes $output_dir/with_gm12878_bias_final_model/deepshap $cell_line $gpu $fold
        bash $PWD/src/evaluation/interpret/interpret_weight.sh $output_dir/with_gm12878_bias_final_model/$model_name.$fold $bed_file xab $data_dir/tiledb/db $chrom_sizes $output_dir/with_gm12878_bias_final_model/deepshap $cell_line $gpu $fold
        bash $PWD/src/evaluation/interpret/interpret_weight.sh $output_dir/with_gm12878_bias_final_model/$model_name.$fold $bed_file xac $data_dir/tiledb/db $chrom_sizes $output_dir/with_gm12878_bias_final_model/deepshap $cell_line $gpu $fold
    done

    python $PWD/src/evaluation/interpret/combine_shap_pickle.py --source $output_dir/with_gm12878_bias_final_model/deepshap --target $output_dir/with_gm12878_bias_final_model/deepshap --type 20k
    cp $PWD/$cur_file_name $output_dir/with_gm12878_bias_final_model/deepshap
...
#python $PWD/src/evaluation/invivo_footprints/tf_modiscohits.py --outdir=$output_dir/with_gm12878_bias_final_model/deepshap \
#        $output_dir/with_gm12878_bias_final_model/deepshap/20K.fold0.deepSHAP \
#        $modisco_output_dir/$cell_line/$setting"_gm12878biasfit"/20K.fold0.deepSHAP.profile_shap.hdf5 \
