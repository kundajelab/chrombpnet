...
def fetch_per_fold_preds(odir,model_path, encid, i, name):
	...
	input_h5 = os.path.join(odir, name+"_w_bias_all_with_ccre_predictions.h5")
	...
	if os.path.isfile(input_log):
		log_paths.append((input_log, "logs.pred.chrombpnet.fold_"+str(i)+"."+encid+".stdout_v3.txt"))
	...
	if os.path.isfile(input_log):
		log_paths.append((input_log, "logs.pred.chrombpnet.fold_"+str(i)+"."+encid+".stderr_v3.txt"))
...
def fetch_pred_tar(encid, args_json, model_paths, name):
	...
	bedn = pd.read_csv("/oak/stanford/groups/akundaje/projects/chromatin-atlas-2022/chrombpnet/folds/DNASE/"+name+"/preds_upload/fold_0/"+name+"_w_bias_all_regions.bed", sep='\t', header=None)
...
for name in names:
	ofile = "/oak/stanford/groups/akundaje/projects/chromatin-atlas-2022/chrombpnet/folds/DNASE/"+name+"/preds_upload/average_preds_with_ccre_vf/"+name+".mean_preds_w_bias.stat"
