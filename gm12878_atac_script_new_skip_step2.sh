...
cur_file_name="gm12878_atac_script_new_skip_step2.sh"
...
if [[ -d $output_dir/final_model_step3_new_skip_step2 ]] ; then
    echo "skipping step 3"
else
    mkdir $output_dir/final_model_step3_new_skip_step2
    echo -e "json_string\t"$step2_bias_json"\nweights\t"$step2_bias_weights"\ncounts_loss_weight\t"$counts_loss_weight_step3"\nprofile_loss_weight\t1\nfilters\t"$filters"\nn_dil_layers\t"$n_dil_layers > $output_dir/final_model_step3_new_skip_step2/params.txt
    params=$output_dir/final_model_step3_new_skip_step2/params.txt
    for fold in 0
    do
        ./main_scripts/final_model_step3_new/train.sh $fold $gpu $model_name $seed $output_dir/final_model_step3_new_skip_step2 $params  $data_dir/tiledb/db $cell_line $PWD/main_scripts/final_model_step3_new/profile_bpnet_dnase_with_bias.py
        ./main_scripts/final_model_step3_new/predict.sh $fold $gpu $model_name $seed  $output_dir/final_model_step3_new_skip_step2  $data_dir/tiledb/db $cell_line $chrom_sizes
        ./main_scripts/final_model_step3_new/score.sh $output_dir/final_model_step3_new_skip_step2 $model_name $fold $cell_line $seed
    done
    cp $PWD/$cur_file_name $output_dir/final_model_step3_new_skip_step2
fi


## UNPLUG MODEL

if [[ -d $output_dir/final_model_step3_new_skip_step2/unplug ]] ; then
    echo "skipping unplugging"
else
    mkdir $output_dir/final_model_step3_new_skip_step2/unplug
    unplug_bias_json=$output_dir/final_model_step3_new_skip_step2/model.0.arch
    unplug_bias_weights=$output_dir/final_model_step3_new_skip_step2/model.0.weights
    echo -e "json_string\t"$unplug_bias_json"\nweights\t"$unplug_bias_weights"\ncounts_loss_weight\t"$counts_loss_weight_step3"\nprofile_loss_weight\t1\nfilters\t"$filters"\nn_dil_layers\t"$n_dil_layers > $output_dir/final_model_step3_new_skip_step2/unplug/params.txt

    params=$output_dir/final_model_step3_new_skip_step2/unplug/params.txt

    for fold in 0
    do
        CUDA_VISIBLE_DEVICES=$gpu python ./main_scripts/unplug_new/get_model_with_bias_unplugged.py --model_params $params --outf $output_dir/final_model_step3_new_skip_step2/unplug/$model_name.$fold.hdf5 
        ./main_scripts/unplug/predict.sh $fold $gpu $model_name $seed $output_dir/final_model_step3_new_skip_step2/unplug $data_dir/tiledb/db $cell_line $chrom_sizes
        ./main_scripts/unplug/score.sh $output_dir/final_model_step3_new_skip_step2/unplug $model_name $fold $cell_line $seed
    done
    cp $PWD/$cur_file_name $output_dir/final_model_step3_new_skip_step2/unplug
fi
...
if [[ -d $output_dir/final_model_step3_new_skip_step2/unplug/deepshap ]] ; then
    echo "skipping interpretations"
else
    mkdir $output_dir/final_model_step3_new_skip_step2/unplug/deepshap
    bed_file=$data_dir/$cell_line"_idr_split"

    for fold in 0
    do
        ./main_scripts/interpret/interpret.sh $output_dir/final_model_step3_new_skip_step2/unplug/$model_name.$fold.hdf5 $bed_file xaa $data_dir/tiledb/db $chrom_sizes $output_dir/final_model_step3_new_skip_step2/unplug/deepshap $cell_line $gpu $fold
        ./main_scripts/interpret/interpret.sh $output_dir/final_model_step3_new_skip_step2/unplug/$model_name.$fold.hdf5 $bed_file xab $data_dir/tiledb/db $chrom_sizes $output_dir/final_model_step3_new_skip_step2/unplug/deepshap $cell_line $gpu $fold
        ./main_scripts/interpret/interpret.sh $output_dir/final_model_step3_new_skip_step2/unplug/$model_name.$fold.hdf5 $bed_file xac $data_dir/tiledb/db $chrom_sizes $output_dir/final_model_step3_new_skip_step2/unplug/deepshap $cell_line $gpu $fold
    done

    python $PWD/main_scripts/interpret/combine_shap_pickle.py --source $output_dir/final_model_step3_new_skip_step2/unplug/deepshap --target $output_dir/final_model_step3_new_skip_step2/unplug/deepshap --type 20k
    cp $PWD/$cur_file_name $output_dir/final_model_step3_new_skip_step2/unplug/deepshap
...
if [[ -d $modisco_sig_dir/$cell_line/$setting"_new_skip_step2"/ ]] ; then
    echo "modisco dir already exists"
else
    mkdir $modisco_sig_dir/$cell_line/$setting"_new_skip_step2"/
    modisco_dir_final=$modisco_sig_dir/$cell_line/$setting"_new_skip_step2"/
    cp  $cell_line/$setting/final_model_step3_new_skip_step2/unplug/deepshap/20K.fold0.deepSHAP $modisco_dir_final
fi
