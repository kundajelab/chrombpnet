...
CUDA_VISIBLE_DEVICES=$gpu kerasAC_predict_tdb \
		    ...
		    --predictions_and_labels_hdf5 $outdir/predictions.H1ESC.withdups.$weight\counts.$seed\seed.$fold \
		    --load_model_hdf5 $outdir/H1ESC.profile.peaks.only.bpnet.withdups.$weight\counts.$seed\seed.$fold.hdf5 \
		    --tasks ENCSR000EMU \
		    --upsample_threads 1 \
