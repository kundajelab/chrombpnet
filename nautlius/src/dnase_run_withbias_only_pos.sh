...
dillayers=$4
inputlen=$5
bias_model_name=$cell_line/$6
data_type=$7

echo $bias_model_name
cur_file_name="dnase_run_withbias_only_pos.sh"
setting=$cell_line"_"$date"_"$seed"_"$dillayers"_"$inputlen"_"$fold_num"_only_pos_hepg2_transfer"
...
if [[ -d $output_dir/chrombpnet_model ]] ; then
    ...
    bash step6_train_chrombpnet_model_dillayer_only_pos.sh $ref_fasta $data_dir"/"$cell_line"_unstranded.bw" $overlap_peak $data_dir/$negatives_bed $fold $bias_model_name $output_dir/chrombpnet_model $data_type $seed $dillayers $inputlen
fi
