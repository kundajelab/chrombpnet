...
setting=$cell_line"_"$date"_"$seed"_"$dillayers"_"$inputlen"_"$fold_num"_samepad"
...
if [[ -d $output_dir/chrombpnet_model ]] ; then
    ...
    bash step6_train_chrombpnet_model_dillayer_same_pad.sh $ref_fasta $data_dir"/"$cell_line"_unstranded.bw" $overlap_peak $data_dir/$negatives_bed $fold $bias_model_name $output_dir/chrombpnet_model $data_type $seed $dillayers $inputlen
fi
