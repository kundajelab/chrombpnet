...
cur_file_name="atac_run.sh"
setting=$cell_line"_"$date"_bias_"$bias_filters"_"$bias_dil"_"$seed"_"$bias_threshold_factor"_fold_"$fold_num"_data_type_"$data_type
...
if [[ -d $output_dir/bias_model ]] ; then
    ...
    bash step4_train_bias_model.sh $ref_fasta $data_dir"/"$cell_line"_unstranded.bw" $overlap_peak $data_dir/$negatives_bed $fold $bias_threshold_factor $output_dir/bias_model $bias_filters $bias_dil $seed
fi
...
if [[ -d $output_dir/chrombpnet_model ]] ; then
    ...
    bash step6_train_chrombpnet_model.sh $ref_fasta $data_dir"/"$cell_line"_unstranded.bw" $overlap_peak $data_dir/$negatives_bed $fold $output_dir/bias_model/bias.h5 $output_dir/chrombpnet_model $data_type $seed
fi
